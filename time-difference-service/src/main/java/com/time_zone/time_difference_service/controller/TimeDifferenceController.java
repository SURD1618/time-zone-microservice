...
import com.time_zone.time_difference_service.client.TimeZoneFeignClient;
...
public class TimeDifferenceController {
    ...
    @Autowired
    private TimeZoneFeignClient timeZoneFeignClient;
    ...
    public TimeDifferenceResponse getTimeDifference(@PathVariable String region1, @PathVariable String region2) {
        ...
        TimeZoneResponse region1Data = timeZoneFeignClient.getTimeZoneByRegion(region1);
        TimeZoneResponse region2Data = timeZoneFeignClient.getTimeZoneByRegion(region2);

        double timeDifference = Math.abs(region1Data.getUtcOffset() - region2Data.getUtcOffset());
